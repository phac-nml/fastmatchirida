...
- The scheduled pipelines output mode (`--output_type scheduled`) now outputs only the original FastMatch XLSX file format. An additional file (`process/fastmatch.scheduled.tsv`) is now generated as described in v0.5.0 as scheduled mode ouput that is used exclusively for parsing schedule pipelines metadata back to IRIDA Next. [PR 53](https://github.com/phac-nml/fastmatchirida/pull/53)
